...
DesplegarOpcionesArchivo macro menuAgenda
    local salirDOA, opcion2DOA, opcion3DOA,llamarIngresarRecordatorio, llamarDesplegarRecordatorio,llamarBorrarRecordatorio,pSalirDOA

    LimpiarPantalla
    gotoxy 0,0

    ImprimirMensaje menuAgenda 
    LeerTecla opcionAgenda
    mov al, opcionAgenda
    sub al, 30h
    
    cmp al, 1
    jne opcion2DOA
    jmp llamarIngresarRecordatorio

opcion2DOA:  
    cmp al, 2
    jne opcion3DOA
    jmp llamarDesplegarRecordatorio 

opcion3DOA:
    cmp al, 3
    jne pSalirDOA
    jmp llamarBorrarRecordatorio 

pSalirDOA:
    jmp salirDOA
llamarIngresarRecordatorio:
    IngresarRecordatorio
    jmp salirDOA 
llamarDesplegarRecordatorio:
    DesplegarRecordatorio
    jmp salirDOA

llamarBorrarRecordatorio: 
    BorrarRecordatorio

salirDOA:
EndM 

IngresarRecordatorio macro
    local forIR, PuenteErrorIR, seguirIR, errorIR, salirIR 

    LimpiarPantalla
    gotoxy 0,0

    ImprimirMensaje solicitarFecha
    ...
    ;crea un archivo y retorna handle.
	mov  ah,3ch
	mov  dx,offset file
    xor  cx,cx 
    int  21h
...
seguirIR: 
    ...
    ImprimirMensaje solicitarRecordatorio 
    xor ax,ax 
    mov  ah,0ah
    mov  dx,offset recordatorio
    int  21h

    mov si, 0
    mov cl, recordatorio[si+1]
    add cl, 2
    mov recordatorio[si], '-'
    mov recordatorio[si+1], ' '
    ...
    ; Cerrar el archivo
    mov ah, 3Eh
    mov bx, handle2  ; Identificador de archivo
    int 21h
    jmp salirIR
...
DesplegarRecordatorio macro 
    local forDR,otrocaracterDR,cerrarDR,salirDR,PuenteErrorDR

    ;FUNCIONALIDAD PARA ABRIR LOS RECORDATORIOS DE UNA FECHA
    LimpiarPantalla 
    gotoxy 0,0
    ImprimirMensaje solicitarFecha
	...
	;abre un archivo y retorna handle.
	mov  ah,3dh
	mov  dx,offset file
    xor  al,al
    int  21h
    ...
    ImprimirMensaje msjRecordatorio
    jmp otrocaracterDR 
PuenteErrorDR: 
    jmp errorDR
otrocaracterDR:             ;ciclo para desplegar todo el archivo.     
    ;lee un archivo handle.
	mov  ah,3fh
	mov  bx,handle 
    ...
    ;imprimir archivo de uno en uno.
    mov  eof,ax
    cmp eof,00h
    je cerrarDR
    mov  ah,02h
...
BorrarRecordatorio macro 
    local forBR, errorBR, salirBR 
...
GetCommanderLine2 macro LineCommand
    local LongLC 
...
CalcularNumeroSemana macro mes, anio, nDiasPorMes, nSemana, fila, columna,num1,num2
...
ImprimirMes macro primerDia, mes, anio, nDiasPorMes, nDia, fila, columna,num1,num2,signo
    local imprimirNS,whileIM, breakIM, salteIM, sigaIM, noIncrementeCol,noImprimirNegativo,noIncrementarFebrero,noHacerAjuste1582
    PushA                           ;se guardan los registros en la pila
...
noImprimirNegativo:     
    ...
    mov columna,1                   ;se coloca la columna en 0     
    ...
    PushA
    CalcularNumeroSemana mes, anio, nDiasPorMes, nDia, fila,columna,num1,num2
    mov num1,ax
    PopA
    ...
    noHacerAjuste1582:
        ...
        noIncrementeCol:            ;etiqueta para saltarse el incremento cuando no es necesario
            ...
            je imprimirNS
            jmp salteIM             ;de no serlo, brinque al final
...
ImprimirAnio2 macro primerDia, mes, anio, nDiasPorMes, nDia, fila, columna,num1,num2,signo
    local noIncFila,noSaltoLinea,ajustarIA,noImprimir,noHacerAjuste1582,imprimirConAjuste1582,puenteNoHacerAjuste1582,noAjustarOctubre,noAjusteInicialFila,ajusteInicialFila,verificarPrimerAjuste, whileIA, noBreakIA, breakIA, whileIA2, noBreakIA2, breakIA2

    mov mes,1                       ;le mueve al mes un 1, para que empiece a imprimir desde enero
    xor cx,cx                       ;limpia el cx
    
    mov columna,0                   ;se coloca el cursor en la pos 0,0
    mov fila,0
    gotoxy fila,columna

    ImprimirMensaje dias2           ;se imprime el mensaje que de los dias de la semana
    ImprimirMensaje dias2
    ImprimirMensaje dias2

    mov columna,5                   ;se coloca la posicion inicial, en la 5,1
    inc fila 
    mov num2,0                      ;se limpia el num2
    ...
    verificarPrimerAjuste:
        mov bx,offset nDiasPorMes       ;coloca el bx en el offset de la cantidad de dias por mes
        mov si,mes                      ;se almacena el numero del mes en di
        ...
        PushA                           ;se guardan los registros en la pila 
        sub mes,3                       ;se le resta tres al mes, para compararlo con el que va arriba de el en el print
        ObtenerJulianDay mes,anio,num1,num2,signo       ;se obtiene el dia juliano de la fecha ingresada, para saber el primer dia del mes
        add mes,3                       ;se reinicia la cantidad que tenia el mes
        mov dh,0            
        mov num1,dx                     ;se guarda el dia de la semana en num1
        PopA                            ;se recuperan los registros
        mov dx,num1                     
    ...
    noAjusteInicialFila:
        ...
        mov bx, offset ene                  ; Dirección base de la tabla de nombres de meses
        add bx, ax                          
        mov al,columna                      
        sub columna,5
        gotoxy fila, columna 
...
CalcularJulianDay macro mes,anio,num1,num2,signo
...
ImprimirConversionAJuliano macro mes,anio,signo
    CalcularJulianDay mes,anio,num1,num2,signo      ;se calcula el dia juliano
    mov num1,ax                                     ;se guarda la parte mas significativa en num1
    mov num2,dx                                     ;y la menos significativa en num2
    dec num2                                        ;se decrementa para ajustarlo
    mov columna,0                                  
    mov fila,0  
    gotoxy fila, columna                            ;se coloca el cursor en la pos 0,0
    ImprimirMensaje msjConvJD                       ;se imprime el mensaje del conversor
    mov columna,35                                  
    gotoxy fila, columna                            ;se coloca el cursor en la pos 0,35
    PrintHex num2,fila,columna                      ;se imprime la parte menos significativa
    PrintHex num1,fila,columna                      ;y la mas significativa

EndM 

ObtenerJulianDay macro mes,anio,num1,num2,signo
    CalcularJulianDay mes,anio,num1,num2,signo
...
CalcularPosColumna macro nDia,columna 
    ...
    xor si,si                               ;se limpia el si
    xor dx,dx                               ;se limpia el dx
    mov columna,5                           ;se inicializa la columna en 0
